...
# stock_share_list.py
# Date : 29/08/2019
# implement linklist according to requrement for modification in company share and update JSON file
#---------------------------------------------------------------------------------------------
...
class stock_function:
 ...
 # if in real time share and amount of stock change then we call this method   
    def edit_details(self,object_name,record, share, amount):
        ...
        if self.head == None:
            ...
            while(temp != None and temp.data != object_name):
                temp = temp.next
 ...
 # Method for print list data (comany name (symbol))   
  ...
  # print total value of stocks in record of all company
   ...
   # search given stock name is present or not in record (Database)
    def search_stock(self,stock_name):
        ...
        while temp != None and temp.data != stock_name:
            temp = temp.next
  ...
  #Mehod for delete item (data) from linked list (we insure data is present in record)
    def delete_item(self, item):
        ...
        while temp.next != None and temp.next.data != item:
            temp = temp.next
 ...
 # buy method is used for add the new stock dertail into the record
    def buy_stock(self,stock_name,amount,record):
        ...
        #update the file with new rocord
        obj.writeFile(record)

 #Print all data of comany share
